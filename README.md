...
3. Go to the next empty column in the same row (in our case, W8), and copy\paste this formula in
    ```=SUBSTITUTE(SUBSTITUTE(SUBSTITUTE($U$8,"WTR006",CONCATENATE("WTR", TEXT(V8, "000"))),"WTR6",CONCATENATE("WTR", V8)),"WTR_6",CONCATENATE("WTR_", V8))```
...
10. Note: You can use something like this for Variations: `SUBSTITUTE($T$8,"WTR006",CONCATENATE("WTR", TEXT(V8, "000")))`
...
| Keywords | string[] | Any keywords that appear on the card. See below for a list of possible keywords. (Include keywords that are conditionally granted to the card, but not that the card grants or removes from other cards.) | Combo, Go again |
