...
def generate_json_file(language):
    ...
    englishJsonPath = Path(__file__).parent / "../../../json/english/card.json"
    csvPath = Path(__file__).parent / f"../../../csvs/{language}/card.csv"
    jsonPath = Path(__file__).parent / f"../../../json/{language}/card.json"

    with csvPath.open(newline='') as csvfile:
        with englishJsonPath.open(newline='') as jsonfile:
            english_card_array = json.load(jsonfile)
